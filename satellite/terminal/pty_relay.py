"""PTY-backed process spawning for interactive CLI sessions — SATELLITE side.

Satellite-owned (NOT a sync-script vendor): this is the **Unix** PTY backend
(``pty.openpty`` + ``fcntl``/``termios`` + asyncio ``add_reader``), adapted from
the proxy's ``core/sandbox/pty_relay.py`` (the proxy keeps its own copy for
LOCAL interactive sessions). The satellite uses it to host a remote interactive
TUI the proxy drives over the WS
(``pty_open``/``pty_input``/``pty_output``/``pty_resize``/``pty_close``).
``pty_session`` imports this LAZILY and only on a non-Windows host; Windows uses
the sibling backend ``winpty_relay`` (ConPTY via ``pywinpty``, with a
reader-thread instead of ``add_reader``).
...
spawn minus ``-p``/stream-json, plus ``TERM``) and the per-session orchestration
live in ``pty_session_base``/``pty_session``. The child runs as a direct
subprocess of the daemon — there is no sandbox on the satellite.
...
Host quirks handled here:
  * The controlling terminal is established BY HAND in the forked child
    ...
    Python 3.11+ and the satellite supports 3.10.
  * ``TERM`` must be present in the child env; callers include it (with a
    logged ``xterm-256color`` fallback here).

Ships a bare PTY + a server-side scrollback ring; tmux-backed persistence
(surviving a daemon restart) and multi-viewer are not implemented — the spawn
seam here stays clean for them.
"""
...
class PtyProcess:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed = False
        # The output reader is a TASK (not a fire-and-forget add_reader
        # callback) so it can AWAIT the backpressured lane put — pausing the read
        # when the WS is congested/disconnected instead of dropping bytes.
    ...
    def _attach(self) -> None:
        """Start the output reader task (called once by spawn_pty)."""
        os.set_blocking(self.master_fd, False)
        self._reader_task = self._loop.create_task(self._reader_loop())

    # -- output: child -> proxy ----------------------------------------------
    async def _reader_loop(self) -> None:
        """Forward PTY output WITH backpressure: each chunk is AWAITED
        onto the proxy-bound lossless lane, so when the lane (the WS) is congested
        or disconnected the read pauses → the kernel PTY buffer fills → the TUI's
        own write() blocks, instead of us dropping bytes. Replaces the old
        fire-and-forget add_reader callback."""
        try:
            while not self._closed:
                ...
                while True:
                    try:
                        ...
                        break  # drained; wait for the next readable edge
            ...
            return
        except Exception:  # never let the reader task take down the loop
            logger.exception("pty %s: reader loop error", self.pid)
            self._on_eof()

    async def _wait_readable(self) -> None:
        """Suspend until the master fd is readable (one-shot add_reader → future)."""
        ...
        self._loop.add_reader(self.master_fd, _ready)
        try:
            await fut
        finally:
    ...
    def inject_output(self, data: bytes) -> None:
        ...
        printed them: remembered in scrollback (so attach-replay keeps them) and
        forwarded on the same lane. For one-shot notices (e.g. the degraded-
        resume banner) — call right after spawn, before the child's first
        output, so ordering is trivially preserved."""
        self._remember(data)
        out = self._on_output(data)
        if asyncio.iscoroutine(out):
    ...
    def close(self, *, signal_child: bool = True) -> None:
        ...
        # Remove any one-shot reader left armed by _wait_readable. Single-threaded
        # loop → the reader task is suspended at an await here, so this can't race a
        # live read, and the cancelled task never touches the fd again.
        with contextlib.suppress(Exception):  # pragma: no cover - loop teardown races
            self._loop.remove_reader(self.master_fd)
        if self._write_buf:
            logger.warning(
...
def spawn_pty(
    ...
    *,
    ...
    def _preexec() -> None:
        ...
        # (os.login_tty is 3.11+; the satellite supports 3.10). ONLY
        # async-signal-safe syscalls here — no allocation/logging
        # (fork-in-a-threaded-process rule). The
